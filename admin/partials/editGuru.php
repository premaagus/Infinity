<?php 
	...
	$queryUser = $koneksi->query("SELECT * FROM tb_user AS a INNER JOIN tb_guru AS b ON a.id_user = b.id_user WHERE a.id_user = $id_user");
...
<form action="" method="POST" enctype="multipart/form-data">
	<div class="container1 d-flex f-row">
		...
		<div class="form-control">
			<p>NIK</p>
			<input type="number" name="nik" id="nik" placeholder="Input NIK Disini..." required value="<?php echo $dataUser['nik'] ?>">
			<div class="alert-err">
				<p>NIK tidak Boleh Kosong</p>
				<div class="point-err"></div>
			</div>
		</div>
			...
			<select name="bidang_ilmu" id="bidang_ilmu">
				<?php 
					...
					while ($dataMapel = $queryMapel->fetch_assoc()) {
						?>
						<option value="<?php echo $dataMapel['nama_mapel'] ?>" <?php if($dataUser['bidang_ilmu'] == $dataMapel['nama_mapel']){echo 'selected';} ?>><?php echo $dataMapel['nama_mapel'] ?></option>
						<?php
					}
				 ?>
			</select>
...
<?php 
	if (isset($_POST['btn-submit'])) {
		...
		if ($queryUpdateUser) {
			$queryUpdateSiswa = $koneksi->query("UPDATE tb_guru SET nama_lengkap = '$nama_lengkap', nik = '$nik', jenis_kelamin='$jenis_kelamin', tempat_lahir='$tempat_lahir', tanggal_lahir='$tanggal_lahir', agama='$agama', telp='$telp', alamat='$alamat', bidang_ilmu='$bidang_ilmu' WHERE id_user = $id_user ");
			if ($queryUpdateSiswa) {
				?>
					<script>
